...
spline=function(data)
{
  ...
  cat("======================================================================= \n")
  cat("      TITIK \n")
  cat(" KNOT[1]   KNOT[2]     KNOT[3]       MSE               GCV            R2 \n")
  cat("======================================================================= \n")
  for(j in 1:d)
  {
    v1=matrix(0,n,p+1)
    ...
    R2[j]=1-(JKG/JKT)
    cat("  ",format(w[j,1]),"      ",format(w[j,2]),"      ",format(w[j,3]),"      ","\t",format(MSE[j]),"\t ",format(GCV[j]),"\t ",format(R2[j]),"\n")
  }
  s<-matrix(c(w[,1],w[,2],w[,3],MSE,GCV,R2),d,6)
  GCVmin<-min(GCV)
  knot<-rep(0,jk)
  knot[1]<-s[s[,5]==GCVmin,1]
  knot[2]<-s[s[,5]==GCVmin,2]
  knot[3]<-s[s[,5]==GCVmin,3]
  mse<-s[s[,5]==GCVmin,4]
  rr<-s[s[,5]==GCVmin,6]
  cat("======================================================================= \n")
  cat(" Berdasarkan data diatas dapat diperoleh : \n")
  cat(" GCV Minimum = ",format(GCVmin),"\n")
  cat(" Pada : titik knot [1] =",knot[1],"\n")
  cat("        titik knot [2] =",knot[2],"\n")
  cat("        titik knot [3] =",knot[3],"\n \n")
  knot=seq(1,d,1)
